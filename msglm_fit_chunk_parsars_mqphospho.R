...
#job.args <- c("cov2", "ast_parsars_mqphospho", "mq_pho_dda_20201006", "20201007", "20201007", "0", "39")
if (!exists('job.args')) {
...
#job_chunk <- filter(mutate(modelobjs_df, chunk=row_number()), str_detect(ptmn_label, "^Phospho_SARS_CoV2_M_S213_M1"))$chunk
...
msdata.df <- dplyr::filter(msdata$ptmn2pepmodstate, ptmn_id %in% sel_object_ids) %>%
  dplyr::inner_join(dplyr::select(msdata$pepmodstate_intensities, evidence_id, pepmodstate_id, msrun,
                                  intensity, psm_pvalue, ident_type)) %>%
  dplyr::left_join(dplyr::select(msdata$ptmn_locprobs, ptmn_id, ptm_locprob, evidence_id, pepmodstate_id, msrun)) %>%
  dplyr::inner_join(dplyr::select(msdata$msruns, msrun, condition, treatment, timepoint)) %>%
  dplyr::filter((coalesce(psm_pvalue, 1) <= data_info$pvalue_max) &
                  (coalesce(ptm_locprob, 1) >= data_info$locprob_min)) %>% # filter valid PTM localization and identification probabilities
...
# aggregate multiple evidences of the same pepmodstate in the same msrun
msdata_agg.df <- dplyr::group_by(msdata.df, ptmn_id, pepmodstate_id, msrun) %>%
...
model_data$subobjects <- msdata_agg.df %>%
  ...
  dplyr::inner_join(cluster_msprofiles(msdata_agg.df, msdata$msrun_pepmodstate_stats, obj_col='pepmodstate_id', msrun_col='msrun')) %>%
