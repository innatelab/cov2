...
object_contrasts_4show.df <- object_contrasts.df %>% #filter(std_type == "replicate") %>%
    ...
    dplyr::mutate(show_label = if_else(rep.int(sum(show_label) >= 600L, n()), rep.int(FALSE, n()), show_label)) %>%
    dplyr::ungroup()
...
sel_pepmodstates.df <- dplyr::inner_join(sel_objects.df, msdata_full[[str_c(modelobj, "2pepmodstate")]]) %>%
    dplyr::filter(is_specific) %>%
    dplyr::inner_join(select(msdata_full$pepmodstates, pepmodstate_id, pepmod_id, charge)) %>%
    dplyr::inner_join(select(msdata_full$pepmods, pepmod_id, peptide_id, mod_seq, unmod_seq=seq)) %>%
    dplyr::select(object_id, pepmodstate_id, pepmod_id, majority_protein_acs, protac_label, gene_label, gene_names,
                  mod_seq, unmod_seq, charge)
if (exists("fit_stats") && has_name(fit_stats, "subobjects")) {
    ...
    sel_pepmodstates.df <- dplyr::arrange(sel_pepmodstates.df, object_id, unmod_seq, mod_seq) %>%
        mutate(pms_median_log2 = NA_real_)
}
sel_pepmodstates.df <- dplyr::mutate(sel_pepmodstates.df,
                                pepmod_ext = paste0(mod_seq, " (", pepmodstate_id, ").", charge,
...
sel_pepmod_intens.df <- dplyr::inner_join(dplyr::select(sel_pepmodstates.df, object_id, protac_label, gene_label, gene_names,
    ...
    dplyr::mutate(intensity_norm = exp(-coalesce(total_msrun_shift, 0.0))*intensity,
                  msrun = msrun.2,
                  msrun.2 = NULL) %>%
...
group_by(sel_pepmod_intens.df, object_id) %>% do({
    ...
    p <- ggplot(shown_pepmod_intens.df) +
        geom_tile(aes(x=sample, y=pepmod_ext,
    ...
    #message(fname)
    ggsave(filename = fname,
           plot = p, width=10, height=3 + n_distinct(shown_pepmod_intens.df$object_id) + min(20, 0.1*n_distinct(shown_pepmod_intens.df$pepmod_id)),
