...
vertex_walkweights = Matrix{Float64}(undef, (size(vertex_weights_perms, 1), length(chunk_treeixs)))
...
for (i, permix) in enumerate(chunk_treeixs)
    ...
    vertex_weights = convert(Vector{W}, view(vertex_weights_perms, :, permix))
