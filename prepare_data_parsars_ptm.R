...
data_info <- list(project_id = project_id,
                  ...
                  effect_damp_factor = 0.99, # how much the scale is applied to the effects that were activated X hours before
...
msdata$ptm2gene <- dplyr::filter(msdata_full$ptm2gene, ptm_type %in% c("GlyGly"))#c("Phospho", "GlyGly"))
...
pepmodstate_intensities.df <- msdata_full$pepmodstate_intensities %>%
  ...
  dplyr::filter(!(msrun %in% bad_msruns)) %>%
  dplyr::group_by(dataset, pepmodstate_id) %>%
...
dplyr::group_modify(~{
  message("Normalizing infection effect scales ", .y$dataset, " dataset,",
          " infected=", .y$infected, ", timepoint=", .y$timepoint,
          "h (", nrow(.x), " effect pair(s))...")
  fit_effect_scales(.x)
})
...
effect_scales_plot <- ggplot(effect_scales.df,
  ...
  scale_y_log10() +
  facet_grid(dataset ~ timepoint, scales="free_x") +
  theme_bw_ast() +
