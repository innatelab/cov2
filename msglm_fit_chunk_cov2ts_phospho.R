...
#job.args <- c("cov2", "ast_cov2_msglm", "cov2timecourse_phospho_dia_20200423", "20200427", "20200427", "0", "1235")
if (!exists('job.args')) {
...
} else if (modelobj == "ptmgroup") {
  msdata.df <- dplyr::filter(msdata$ptmgroups, ptmgroup_id %in% sel_object_ids) %>%
    dplyr::inner_join(dplyr::select(msdata$ptmgroup_intensities, ptmgroup_id, msrun, intensity)) %>%
...
model_data$objects <- dplyr::mutate(model_data$objects, protgroup_id = object_id) %>%
  ...
  model_data$objects <- dplyr::mutate(model_data$objects, ptmgroup_id = object_id) %>%
    dplyr::inner_join(dplyr::select(modelobjs_df, ptmgroup_id, object_label,
                                    majority_protein_acs, gene_names, protein_names, contains("is_"))) %>%
    #dplyr::select(-is_fit) %>%
    dplyr::arrange(glm_object_ix)
}
