...
data_info <- list(project_id = project_id,
                  ...
                  msfolder = msfolder,
                  mscalib_protgroup_filename = "instr_protgroup_LFQ_calib_scaturro_qep5calib_20161110_borg.json",
                  mscalib_pepmodstate_filename = "instr_QEP5_intensity_pepmodstate_calib_cov2_20200430.json",
...
mscalib_pepmodstate <- mscalib_pepmodstate_json$instr_calib
...
msdata_full$msruns <- msdata_full$msruns %>%
    dplyr::mutate(dataset = factor("phospho", c("phospho", "ubi")),
...
msdata_full$pepmodstate_intensities <- dplyr::inner_join(dplyr::rename(msdata_full$pepmodstate_intensities, mq_msrun=msrun),
                                                         dplyr::select(msdata_full$msruns, dataset, mq_msrun, msrun)) %>%
  dplyr::mutate(ident_type = factor(ident_type, maxquantUtils:::MaxQuant_IdentTypes, ordered = TRUE),
                mq_msrun=NULL)

# msdata_full$pepmodstate_intensities <- group_by(msdata_full$pepmodstate_intensities, pepmodstate_id, msrun, dataset) %>%
#   dplyr::summarise(intensity = sum(intensity, na.rm=TRUE),
#                    ident_type = min(ident_type, na.rm=TRUE),
#                    psm_pvalue = min(psm_pvalue, na.rm=TRUE),
#                    .groups = "drop") %>%
#   dplyr::mutate(intensity = if_else(intensity == 0, NA_real_, intensity),
#                 psm_pvalue = if_else(is.infinite(psm_pvalue), NA_real_, psm_pvalue))

msdata_full$ptmn_locprobs <- dplyr::inner_join(dplyr::rename(msdata_full$ptmn_locprobs, mq_msrun=msrun),
                                              dplyr::select(msdata_full$msruns, dataset, mq_msrun, msrun)) %>%
  dplyr::select(-mq_msrun)# %>%
  #dplyr::group_by(protein_ac, peptide_id, pepmodstate_id, ptm_type, ptm_AA_seq, ptm_offset, peptide_pos, ptm_pos, dataset, msrun) %>%
  #dplyr::summarise(ptm_locprob = max(ptm_locprob, na.rm = TRUE), .groups = "drop")
...
msdata_full$ptmn_stats <- inner_join(msdata_full$ptmns, dplyr::select(msdata_full$ptmn2pepmodstate, ptmn_id, pepmodstate_id)) %>%
  dplyr::inner_join(dplyr::select(msdata_full$pepmodstate_intensities, evidence_id, pepmodstate_id, dataset, msrun, intensity, ident_type, psm_pvalue)) %>%
...
msdata$pepmodstate_intensities <- msdata_full$pepmodstate_intensities %>% filter((coalesce(psm_pvalue, 1.0) <= data_info$pvalue_max) &
                                                                                 (ident_type != "MULTI-MATCH-MSMS")) %>%
...
msdata4norm.df <- msdata_full$pepmodstate_intensities %>% ungroup() %>%
  dplyr::filter(!is.na(intensity) & coalesce(psm_pvalue, 1) <= 1E-4) %>% 
...
#msruns_hnorm <- readRDS(file.path(scratch_path, str_c(project_id, "_", msfolder, "_20201007_raw.rds")))
...
norm_plot <- ggplot(inner_join(msdata_full$pepmodstate_intensities, msdata_full$msruns) %>%
                    inner_join(total_msrun_shifts.df), aes(x=msrun)) +
    geom_violin(aes(y = total_msrun_shift/log(2)-log2(normfactor)), draw_quantiles=c(0.25, 0.75), scale="width", fill="lightgray") +
    facet_wrap(~ treatment + dataset, ncol=2, scales = "free") +
    theme_bw_ast(base_family = "") +
    theme(axis.text.x = element_text(hjust = 0, angle=-45))
