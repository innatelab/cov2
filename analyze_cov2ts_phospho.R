...
sel_ptmgroups.df <- dplyr::filter(msdata$ptmgroups, str_detect(ptmgroup_id, "EPHA2_T898"))
...
group_by(sel_ptmgroups.df, ptmgroup_id) %>% do({
    sel_ptmgroup.df <- .
    message("Plotting ", sel_ptmgroup.df$ptmgroup_label, " timecourse")
    sel_ptm_msdata.df <- sel_ptmgroup.df %>%
        dplyr::inner_join(msdata$ptmgroup_intensities) %>%
        dplyr::inner_join(msdata$msruns)
    sel_ptm_iactions.df <- dplyr::inner_join(dplyr::select(sel_ptmgroup.df, ptmgroup_id, ptmgroup_label),
                                             dplyr::filter(fit_stats$iactions, var=="iaction_labu_replCI")) %>%
        dplyr::inner_join(conditions.df) %>%
        dplyr::mutate_at(vars(mean, ends_with("%")),
                         list(~exp(. + global_ptmgroup_labu_shift)))
p <-
ggplot(data=sel_ptm_iactions.df, aes(x = timepoint_num, color=treatment, fill=treatment)) +
    ...
    geom_point(data=sel_ptm_msdata.df,
